...
public:
  ...
  bool IsPartDownload(RequestInfo& info)
  {
    ...
    std::cout << "_etag: " << _etag << "_date: " << _date << std::endl;
    //不是断点续传
  ...
  {
    std::cout << "In ProcessPartDownload" << std::endl;
    std::string range = info._part_list[i];
    if (i == 0)
    {
      //对于发送第一个块，要将bytes=删除掉
    ...
    //std::string begin;
    //std::string finish;
    //Utils::DigitToStr(start, begin);
    //Utils::DigitToStr(end, finish);
    ...
    SendData(rsp_header);
    std::cout << "part:rsp_header\n" << rsp_header << std::endl;

    std::cout << "phys: " << info._path_phys.c_str() << std::endl;
